...
input_data = input('Input rating. If element is -1, input is finished: ')

rating = int(input_data) if input_data !='' else None

while rating is not None:
    my_list.append(rating)
    my_list.sort(reverse = True)
    print(f'Пользователь ввёл число {rating}. Результат: {my_list}.')
    input_data = input('Input rating. If element is -1, input is finished: ')
    rating = int(input_data) if input_data !='' else None